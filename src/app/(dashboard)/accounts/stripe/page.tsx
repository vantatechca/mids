...
type Account = {
  ...
  domainName?: string;
  status: string;
  paymentProcessorConnected: string | null;
...
export default function StripeAccountsPage() {
  ...
  const { data: accounts = [], isLoading } = useQuery<Account[]>({
    queryKey: ["accounts", "stripe"],
    queryFn: () => fetch("/api/accounts?platform=stripe").then((r) => r.json()),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      fetch("/api/accounts", {
        ...
        body: JSON.stringify({ ...data, platform: "stripe" }),
      }).then((r) => r.json()),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["accounts", "stripe"] });
      setSheetOpen(false);
      resetForm();
  ...
  function openEdit(account: Account) {
    ...
    setFormData({
      ...
      paymentProcessorConnected: account.paymentProcessorConnected || "",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Stripe Accounts</h1>
          <p className="text-muted-foreground">Manage Stripe payment accounts</p>
        </div>
                ...
                {filteredAccounts.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} className="text-center py-8 text-muted-foreground">
                      {searchQuery || statusFilter !== "all" ? "No accounts match your filters" : "No Stripe accounts yet."}
                    </TableCell>
                  </TableRow>
                ) : (
      ...
      <Sheet open={sheetOpen} onOpenChange={(open) => { setSheetOpen(open); if (!open) resetForm(); }}>
        <SheetContent className="sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{editingAccount ? "Edit Stripe Account" : "Add Stripe Account"}</SheetTitle>
            <SheetDescription>{editingAccount ? "Update account details" : "Add a new Stripe account"}</SheetDescription>
          </SheetHeader>
          <form onSubmit={handleSubmit} className="space-y-4 mt-6">
            ...
            <div className="space-y-2">
              ...
              <Input id="accountId" value={formData.accountId} onChange={(e) => setFormData({ ...formData, accountId: e.target.value })} placeholder="acct_..." />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="paymentProcessorConnected">Connected Processor</Label>
              <Input id="paymentProcessorConnected" value={formData.paymentProcessorConnected} onChange={(e) => setFormData({ ...formData, paymentProcessorConnected: e.target.value })} placeholder="e.g. Moneris, Square" />
            </div>
            <div className="space-y-2">
